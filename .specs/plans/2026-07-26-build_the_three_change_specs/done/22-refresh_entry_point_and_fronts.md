...
**Implements:** [`changes/merged/2026-07-26-refresh_the_wall.md`](../../../changes/merged/2026-07-26-refresh_the_wall.md) §Proposed changes → `02-feed-engine.md` → Entry point and → Refresh, and → `06-wire-contract.md` → The endpoint; implementation notes 4, 5 and 6. Also owns the cross-spec obligation [`lay_a_bluesky_feed`](../../../changes/merged/2026-07-26-lay_a_bluesky_feed.md) (moved to `changes/merged/` by task 19) assigns to whoever merges second: `refresh` over a feed wall bypasses the `feed_pages` entry.
**Depends on:** 12, 21
...
- [ ] Meets the repo definition of done, plus the explicit `just wasm && cd web && pnpm check:ci` run. That run only means something because task 00 put `src/service-worker.ts` into a tsc project of its own; `web/.svelte-kit/tsconfig.json` excludes the file from the app project, so before task 00 the call site was typechecked against neither a fresh nor a stale `pkg/`. Task 00 also gave `test` its `wasm` dependency, so `just check` rebuilds the engine before it typechecks; the explicit pair here is the faster failure, not the only one.
