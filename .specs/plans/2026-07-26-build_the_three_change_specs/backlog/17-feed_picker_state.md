...
- [ ] Filter out any feed whose own view carries a hidden label, or whose creator does, driving the cases from the `HiddenLabel` type rather than a retyped array.
...
- [ ] There is one vitest case per label in `HiddenLabel`, driven from the type, so a label added to mortar's `HIDDEN_LABELS` is a compile error here before it is a listing bug.
